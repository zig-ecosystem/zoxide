...
- [x] **H20 实测：机制分离出来了，结论与预期相反**（`constbench-div2-20260924`）：
  | | const | ldg | 比值 |
  |---|---|---|---|
  | uniform | 0.251 ms | 0.257 ms | **1.02x（噪声）** |
  | divergent | 0.906 ms | 0.273 ms | **0.30x（慢 3.3×）** |
  - **串行化成立**：0.30x 远低于纯指令数预言的 0.79x，constant 窗口确实按不同地址串行
  - **但 uniform 没有优势**：1.02x。先前那个 1.11x 是 `trips=4` 时启动开销造成的假象
  - 所以「广播缓存所以更快」这个流传甚广的说法**在 H20 上没有通过测量**。
    三处 doc comment 的断言已删，换成实测数字 + 发散会慢 3.3× 的警告
  - `ConstBank` 的定位改为：为 `__constant__` 语义和省参数位而用，不为吞吐
  - 我的判据消息本身也写错了：它把「串行化成立」和「uniform 优势是缓存」绑成一句，
    于是在 uniform 只有 1.02x 时打印了「uniform 的优势是缓存」。已拆成两个独立判定
- [x] ~~待测：分离机制~~（`constbench-div-20260924`，已出数见上）。判据：constant memory 对 warp 内
  ...
  - 第一次跑挂在我自己的 bug 上：`trips` 从 4 提到 64 时只改了 kernel 那份，host 侧
    还有一份副本。`max rel err 15.000009` = `64/4 - 1`，这个数直接点出了成因。
    **计时会照常跑出看起来合理的数字，只有正确性检查拦住了它**
  - 修法不是同步两个常量，而是搬进 `examples_abi.zig` 让漂移变成编译错误 ——
    这个文件的开头本来就写着「声明一次，一边改动另一边就是编译错误」，而我复制了一份。
    顺带把四个 kernel 接上 `cuda.abi.assertMatches`（故意改坏一个参数类型验证过会拦）
